...
export default function DoorStatusPage(){
  ...
  useEffect(()=>{load();const source=new EventSource(`${API_BASE_URL}/api/events/stream`,{withCredentials:true});source.addEventListener('device-status',load);source.onerror=()=>setError('Live device stream disconnected; status will refresh periodically.');const timer=setInterval(load,5000);return()=>{source.close();clearInterval(timer)}},[])
  const actuator=devices.find(d=>d.type==='SIMULATOR'||d.type==='ACTUATOR'), unlocked=actuator?.status==='UNLOCKED'||actuator?.status==='UNLOCKING'
